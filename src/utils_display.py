...
def get_tcp_display(
    printer: Callable[[str], None], display: bool
) -> Callable[[str, str, Packet, Optional[str]], None]:
    def not_print_tcp_request(
        _src_ip: str, _dst_ip: str, _pkt: Packet, _color: Optional[str]
    ) -> None:
        pass

    def print_tcp_request(
        src_ip: str, dst_ip: str, pkt: Packet, color: Optional[str]
    ) -> None:
        ...
        printer(
            f"{color}From\t\t: {src_ip}:{pkt[TCP].sport} -> {dst_ip}:{pkt[TCP].dport}{COLOR_END}"
        )
        printer(f"{color}TS\t\t: {str(timedelta(seconds=elapsed))}{COLOR_END}")
        printer(f"{color}Size\t\t: {len(payload)} bytes{COLOR_END}")
        printer(
            f"{color}Hex\t\t: {binascii.hexlify(payload)[:100]!r}...{COLOR_END}"
        )  # First 50 bytes
        printer("---")
...
def print_proto_name(
    printer: Callable[[str], None], any_msg: google.protobuf.any_pb2.Any
) -> None:
